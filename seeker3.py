...
class Seeker3(Seeker):
    ...
    def _get_best_move(self, my_pos, target, valid_moves):
        ...
        chosen_move = valid_moves[np.argmax(option_distances)]
        if np.sum(np.abs(valid_moves - my_pos)) <= 2:
            self.impatience += 1
        else:
            self.impatience = 0
        
        if self.impatience >= self.MAX_IMPATIENCE:
            # Re-initialize the probability matrix b/c we're stuck
            self.reinit_hider_probabilities_random(3)
