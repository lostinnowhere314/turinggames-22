...
class My_hider(Hider):
    ...
    def _travel_obj(self, valid_moves):
        ...
        if dir == 'north':
            move = valid_moves[np.argmin(np.linalg.norm(v - np.array(self.pos) + np.array([-4, 0]), ord=1) for v in valid_moves)]
        else:
            move = valid_moves[np.argmin(np.linalg.norm(v - np.array(self.pos) + np.array([0, -4]), ord=1) for v in valid_moves)]
        return move
    ...
    def _migrate(self, valid_moves):
        ...
        if self.new_quad is None:
            self._migrate_quad()
        if self.new_quad == 0:
            move = valid_moves[np.argmin(np.linalg.norm(v - np.array([5, 5]), ord=1)
                                         for v in valid_moves)]
        elif self.new_quad == 1:
            move = valid_moves[np.argmin(np.linalg.norm(v - np.array([5, 25]), ord=1)
                                         for v in valid_moves)]
        elif self.new_quad == 2:
            move = valid_moves[np.argmin(np.linalg.norm(v - np.array([25, 5]), ord=1)
                                         for v in valid_moves)]
        else: # quadrand 3
            move = valid_moves[np.argmin(np.linalg.norm(v - np.array([25, 25]), ord=1)
                                         for v in valid_moves)]
        return move

    def _migrate_quad(self):
        if self.pos[0] in range(0, 15) and self.pos[1] in range(0, 15):
            self.quad = 0
        elif self.pos[0] in range(0, 15) and self.pos[1] not in range(0, 15):
            self.quad = 1
        elif self.pos[0] not in range(0, 15) and self.pos[1] in range(0, 15):
            self.quad = 2
        else:
            self.quad = 3
        if random.random() < 0.5:
            self.new_quad = (self.quad + 2) % 4
        else:
            self.new_quad = (self.quad + 3) % 4
